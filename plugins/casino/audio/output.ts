...
async function playFallback(wav: Buffer): Promise<void> {
  if (process.platform === "win32") {
    const script =
      "$m=[IO.MemoryStream]::new();[Console]::OpenStandardInput().CopyTo($m);$m.Position=0;$p=[System.Media.SoundPlayer]::new($m);$p.PlaySync();$p.Dispose();$m.Dispose()"
    if (
      await runProcess("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], wav)
    )
      return
