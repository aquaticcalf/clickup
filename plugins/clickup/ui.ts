...
import { Container, SettingsList, Spacer, Text, matchesKey, type Component, type SettingItem } from "@earendil-works/pi-tui";
...
class SettingsPanel extends Container {
  ...
  constructor(
    ...
    this.list = new SettingsList(items, Math.min(Math.max(items.length, 8), 15), getSettingsListTheme(), onChange, onCancel, {
      enableSearch: true,
    });
...
function actionPanel(menu: MenuContext, title: string, description: string, operation: () => Promise<string>): Component {
...
function permissionPicker(
  ...
  const items: SettingItem[] = ALL_PERMISSIONS.map((permission) => ({
    id: permission,
    label: permission === "r" ? "Read" : permission === "c" ? "Create" : permission === "u" ? "Update" : "Delete",
    description: permission === "r"
      ? "GET requests"
      : permission === "c"
        ? "POST requests"
        : permission === "u"
  ...
  return new SettingsPanel(label, "Toggle permissions, then apply the selection.", items, (id, value) => {
    if (id === "apply") return;
...
function makeAccess(menu: MenuContext): Component {
  ...
  return new SettingsPanel("Access", "Start, narrow, or stop ClickUp access for this session.", [
    {
      id: "permissions",
    ...
    },
    {
      ...
      currentValue: "stop",
      submenu: () => actionPanel(menu, "Stop ClickUp access", "Revoke all permissions immediately.", menu.actions.stop),
    },
  ], () => undefined, menu.done);
}
...
export function createClickUpMenu(
  ...
  return new SettingsPanel("ClickUp", "Manage access without separate start, stop, and logout commands.", [
    {
      id: "access",
    ...
    },
    {
      ...
      currentValue: "view",
      submenu: () => actionPanel(menu, "ClickUp status", "Current local access state.", async () => actions.status()),
    },
    {
      id: "logout",
      label: "Log out",
      ...
      currentValue: "logout",
      submenu: () => actionPanel(menu, "ClickUp logout", "Delete the saved credential and stop access.", menu.actions.logout),
    },
  ], () => undefined, done);
}
