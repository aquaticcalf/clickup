...
export type Runner = (command: string, args: string[], options?: { cwd?: string; signal?: AbortSignal; timeout?: number }) => Promise<ExecResult>;
...
export class GitService {
  ...
  async git(args: string[], cwd: string, signal?: AbortSignal, timeout = 30_000): Promise<ExecResult> {
    return this.run("git", args, { cwd, signal, timeout });
  }

  async checked(args: string[], cwd: string, signal?: AbortSignal, timeout = 30_000): Promise<string> {
  ...
  async status(cwd: string, signal?: AbortSignal): Promise<RepositoryStatus> {
    ...
    const branchResult = await this.git(["symbolic-ref", "--quiet", "--short", "HEAD"], root, signal);
    const upstreamResult = await this.git(["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{upstream}"], root, signal);
    const originResult = await this.git(["remote", "get-url", "origin"], root, signal);
    const status = await this.checked(["status", "--porcelain=v1", "--untracked-files=all"], root, signal);

    let ahead: number | undefined;
    let behind: number | undefined;
    if (upstreamResult.code === 0 && upstreamResult.stdout.trim()) {
      const counts = await this.git(["rev-list", "--left-right", "--count", "HEAD...@{upstream}"], root, signal);
      if (counts.code === 0) {
  ...
  async checkoutPullRequest(number: number, cwd: string, signal?: AbortSignal): Promise<void> {
    const result = await this.run("gh", ["pr", "checkout", String(number), "--detach"], { cwd, signal, timeout: 120_000 });
    if (result.code !== 0) {
      const detail = (result.stderr || result.stdout).trim();
  ...
  async switchBack(state: CheckoutState, cwd: string, signal?: AbortSignal): Promise<void> {
    ...
    if (status.dirtyFiles.length > 0) {
      throw new Error("The PR checkout has changes. Commit or remove them before returning to the original branch.");
    }
  ...
  async pushCurrent(cwd: string, signal?: AbortSignal): Promise<string> {
    ...
    if (result.code !== 0) throw new Error((result.stderr || result.stdout).trim() || "Push failed.");
  ...
  async commitAll(message: string, cwd: string, signal?: AbortSignal): Promise<void> {
    ...
    if (add.code !== 0) throw new Error((add.stderr || add.stdout).trim() || "Could not stage changes.");
    const commit = await this.git(["commit", "-m", message], status.root, signal, 120_000);
    if (commit.code !== 0) throw new Error((commit.stderr || commit.stdout).trim() || "Could not create commit.");
  }
}
