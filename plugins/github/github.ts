...
function checkSummary(rollup: unknown): string {
  ...
  const states = rollup.map((check) => {
    ...
    return String(item.conclusion ?? item.state ?? item.status ?? "unknown").toLowerCase();
  });
  if (states.some((state) => ["failure", "failed", "cancelled", "timed_out", "error"].includes(state))) return "failed";
...
export class GitHubService {
  ...
  async gh(args: string[], cwd: string, signal?: AbortSignal, timeout = 60_000): Promise<ExecResult> {
    return this.run("gh", args, { cwd, signal, timeout });
  }

  async checked(args: string[], cwd: string, signal?: AbortSignal, timeout = 60_000): Promise<string> {
  ...
  async currentPullRequest(cwd: string, signal?: AbortSignal): Promise<{ number: number; title: string; url: string } | undefined> {
  ...
  async listPullRequests(cwd: string, signal?: AbortSignal): Promise<PullRequestSummary[]> {
    const data = parseJson<Record<string, unknown>[]>(
      await this.gh(
        ["pr", "list", "--state", "open", "--limit", "50", "--json", "number,title,author,isDraft,reviewDecision,statusCheckRollup,url,updatedAt,headRefName,baseRefName"],
        cwd,
        signal,
      ),
      "gh pr list",
    ...
    return data.map((item) => ({
      ...
      author: String(value<Record<string, unknown>>(item, "author")?.login ?? "unknown"),
  ...
  async pullRequestDetails(number: number, cwd: string, signal?: AbortSignal): Promise<string> {
    ...
    const author = value<Record<string, unknown>>(item, "author")?.login ?? "unknown";
    const files = Array.isArray(value(item, "files")) ? value<unknown[]>(item, "files")!.length : 0;
    const labels = Array.isArray(value(item, "labels"))
      ? value<unknown[]>(item, "labels")!.map((label) => typeof label === "object" && label ? String((label as Record<string, unknown>).name ?? "") : "").filter(Boolean).join(", ")
      : "";
    const body = String(value(item, "body") ?? "").trim() || "(no description)";
    return [
      `#${number} ${String(value(item, "title") ?? "(untitled)")}`,
      `Author: ${String(author)}${value(item, "isDraft") ? " · draft" : ""}`,
  ...
  async checks(number: number | undefined, cwd: string, signal?: AbortSignal): Promise<string> {
    const args = ["pr", "checks", ...(number ? [String(number)] : []), "--json", "name,state,bucket,link,workflow"];
    ...
    try {
      ...
      return checks.map((check) => {
        const state = String(check.state ?? check.bucket ?? "unknown");
        const name = String(check.name ?? check.workflow ?? "check");
  ...
  async waitForChecks(number: number, cwd: string, signal?: AbortSignal): Promise<string> {
    return this.checked(["pr", "checks", String(number), "--watch", "--interval", "10"], cwd, signal, 30 * 60_000);
  }

  async listIssues(cwd: string, query?: string, signal?: AbortSignal): Promise<IssueSummary[]> {
    const data = parseJson<Record<string, unknown>[]>(
      await this.gh(
        ["issue", "list", "--state", "open", "--limit", "50", ...(query ? ["--search", query] : []), "--json", "number,title,author,labels,url"],
        cwd,
        signal,
      ),
      "gh issue list",
  ...
  async issueDetails(number: number, cwd: string, signal?: AbortSignal): Promise<string> {
    const data = await this.checked(["issue", "view", String(number), "--json", "number,title,body,author,labels,url,state"], cwd, signal);
    const item = JSON.parse(data) as Record<string, unknown>;
    const author = value<Record<string, unknown>>(item, "author")?.login ?? "unknown";
    const labels = Array.isArray(value(item, "labels"))
      ? value<unknown[]>(item, "labels")!.map((label) => typeof label === "object" && label ? String((label as Record<string, unknown>).name ?? "") : "").filter(Boolean).join(", ")
    ...
    return [
      `#${number} ${String(value(item, "title") ?? "(untitled)")}`,
      `Author: ${String(author)} · State: ${String(value(item, "state") ?? "unknown")}`,
      `Labels: ${labels}`,
      `URL: ${String(value(item, "url") ?? "")}`,
      "",
  ...
  async createPullRequest(title: string, body: string, cwd: string, signal?: AbortSignal): Promise<string> {
    return this.checked(["pr", "create", "--title", title, "--body", body], cwd, signal, 120_000);
  }

  async editPullRequest(number: number, args: string[], cwd: string, signal?: AbortSignal): Promise<string> {
    return this.checked(["pr", "edit", String(number), ...args], cwd, signal, 60_000);
  }

  async commentOnPullRequest(number: number, body: string, cwd: string, signal?: AbortSignal): Promise<string> {
    return this.checked(["pr", "comment", String(number), "--body", body], cwd, signal, 60_000);
  }

  async setPullRequestState(number: number, state: "close" | "reopen", cwd: string, signal?: AbortSignal): Promise<string> {
  ...
  async mergePullRequest(number: number, method: "merge" | "squash" | "rebase", cwd: string, signal?: AbortSignal): Promise<string> {
