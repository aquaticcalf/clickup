...
function saveState(pi: ExtensionAPI, settings: PluginSettings, checkout: CheckoutState | undefined): void {
...
export default function github(pi: ExtensionAPI): void {
  ...
  const withStatus = async (ctx: ExtensionCommandContext): Promise<RepositoryStatus> => git.status(ctx.cwd, ctx.signal);

  const confirmChange = async (ctx: ExtensionCommandContext, title: string, message: string): Promise<boolean> => {
  ...
  const prepareCleanCheckout = async (ctx: ExtensionCommandContext): Promise<RepositoryStatus> => {
    ...
    if (status.dirtyFiles.length > 0) {
      if (settings.dirtyPolicy === "refuse") {
        throw new Error(`Working tree is not clean (${status.dirtyFiles.length} changed path(s)). Commit your work before checking out a PR.`);
      }
      ...
      if (!message?.trim()) throw new Error("PR checkout cancelled; no commit message was supplied.");
  ...
  const checkoutPullRequest = async (ctx: ExtensionCommandContext, number: number): Promise<void> => {
    ...
    if (!confirmed) return;
    if (checkout) throw new Error(`Already tracking PR #${checkout.prNumber}. Return from it first.`);
  ...
  const actions = (ctx: ExtensionCommandContext): GithubMenuActions => ({
    ...
    showStatus: async () => {
      ...
      if (ghAvailable) {
        ...
        lines.push(currentPr ? `Current PR: #${currentPr.number} ${currentPr.title}` : "Current PR: none");
      }
      return output(lines.join("\n"));
    },
    showPullRequest: (number) => githubApi.pullRequestDetails(number, ctx.cwd, ctx.signal).then(output),
    ...
    createPullRequest: async () => {
      ...
      const confirmed = await confirmChange(ctx, "Create pull request?", `Create a PR from ${status.branch}?`);
      if (!confirmed) return;
  ...
  const openMenu = async (_args: string, ctx: ExtensionCommandContext): Promise<void> => {
    ...
    await ctx.ui.custom<void>((tui, _theme, _keybindings, done) => createGithubMenu(ctx, settings, actions(ctx), tui, done));
  };
      ...
      async getSuggestions(lines, line, col, options) {
        ...
        return {
          ...
          items: issueCache
            ...
            .slice(0, 20)
            .map((issue) => ({ value: `#${issue.number}`, label: `#${issue.number}`, description: issue.title })),
        };
      },
