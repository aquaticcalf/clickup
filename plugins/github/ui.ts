...
class NumberInputPanel extends Container {
  ...
  constructor(
    ...
    this.input.onSubmit = (value) => {
      void submit(value)
        .then((result) => this.showResult(result ?? "No changes made."))
        .catch((error: unknown) => this.showResult(errorText(error), true))
    }
...
function makeWorkflow(menu: MenuContext): Component {
  return new SettingsPanel(
    "PR workflow",
    "Checkout and restore pull-request code safely.",
    [
      {
        id: "checkout",
        label: "Checkout a PR",
        ...
        submenu: () =>
          prPicker(menu, "Checkout PR", "Select an open pull request", (number) =>
            menu.actions.checkoutPullRequest(number),
          ),
      },
      {
        id: "checkout-number",
      ...
      },
      {
        ...
        submenu: () =>
          actionPanel(menu, "Return from PR", "Restore the original branch.", () =>
            menu.actions.returnFromPullRequest(),
          ),
      },
      {
        id: "refresh",
        ...
        submenu: () =>
          actionPanel(menu, "Refresh PR list", "Reload open pull requests.", () =>
            menu.actions.refreshPullRequests(),
          ),
      },
    ],
    () => undefined,
...
function makeBrowse(menu: MenuContext): Component {
  return new SettingsPanel(
    "Browse GitHub",
    "Inspect pull requests, diffs, issues, and repository links.",
    [
      {
        id: "details",
        label: "PR details",
        ...
        currentValue: "choose PR",
        submenu: () =>
          prPicker(menu, "PR details", "Select a pull request", (number) =>
            menu.actions.showPullRequest(number),
          ),
      },
      {
        id: "diff",
        ...
        currentValue: "choose PR",
        submenu: () =>
          prPicker(menu, "PR diff", "Select a pull request", (number) =>
            menu.actions.showDiff(number),
          ),
      },
      {
        id: "list",
        ...
        currentValue: "view",
        submenu: () =>
          readPanel(menu, "Open pull requests", "Currently open pull requests.", async () => {
            const prs = await menu.actions.listPullRequests()
            return prs.length
              ? prs
                  .map((pr) => `#${pr.number} ${pr.title} · ${pr.author} · ${pr.checks}`)
                  .join("\n")
      ...
      },
      {
        ...
        currentValue: "choose PR",
        submenu: () =>
          prPicker(menu, "Open PR", "Select a pull request", (number) =>
            menu.actions.openPullRequest(number),
          ),
      },
      {
        id: "issues",
        label: "Issues",
        description: "Browse open issues.",
        currentValue: "choose issue",
        submenu: () => issuePicker(menu),
      },
      {
        id: "open-repo",
        ...
        currentValue: "open",
        submenu: () =>
          actionPanel(menu, "Open repository", "Open the repository in your browser.", () =>
            menu.actions.openRepository(),
          ),
      },
    ],
    () => undefined,
...
function makeChecks(menu: MenuContext): Component {
  return new SettingsPanel(
    "Checks and CI",
    "Review and wait for pull-request checks.",
    [
      {
        id: "current",
        ...
        currentValue: "view",
        submenu: () =>
          readPanel(menu, "Current checks", "Checks for the current pull request.", () =>
            menu.actions.showChecks(),
          ),
      },
      {
        id: "pr",
        ...
        currentValue: "choose PR",
        submenu: () =>
          prPicker(menu, "PR checks", "Select a pull request", (number) =>
            menu.actions.showChecks(number),
          ),
      },
      {
        id: "wait",
        ...
        currentValue: "choose PR",
        submenu: () =>
          prPicker(menu, "Wait for checks", "Select a pull request", (number) =>
            menu.actions.waitForChecks(number),
          ),
      },
    ],
    () => undefined,
...
function makeRepository(menu: MenuContext): Component {
  const chooseAnd = (
    title: string,
    operation: (number: number, back: () => void) => Promise<PanelResult>,
  ): Component => prPicker(menu, title, "Select a pull request", operation)

  return new SettingsPanel(
    "Repository actions",
    "Changes stay in this menu; confirmations still protect destructive actions.",
    [
      {
        id: "create",
        label: "Create PR",
        ...
        currentValue: "start",
        submenu: () =>
          actionPanel(menu, "Create PR", "Create a pull request from the current branch.", () =>
            menu.actions.createPullRequest(),
          ),
      },
      {
        id: "push",
        label: "Push current branch",
        description: "Push the current branch after confirmation.",
        currentValue: "push",
        submenu: () =>
          actionPanel(
            menu,
            "Push current branch",
            "Push local commits to the configured upstream.",
            () => menu.actions.pushCurrentBranch(),
          ),
      },
      {
        id: "edit",
        label: "Edit PR",
        description: "Choose a PR and edit its title/body.",
        currentValue: "choose PR",
        submenu: () => chooseAnd("Edit PR", (number) => menu.actions.editPullRequest(number)),
      },
      {
        id: "comment",
        label: "Comment on PR",
        description: "Choose a PR and add a comment.",
        currentValue: "choose PR",
        submenu: () =>
          chooseAnd("Comment on PR", (number) => menu.actions.commentOnPullRequest(number)),
      },
      {
        id: "ready",
        label: "Mark ready for review",
        description: "Choose a draft PR to mark ready.",
        currentValue: "choose PR",
        submenu: () =>
          chooseAnd("Mark ready", (number) => menu.actions.markPullRequestReady(number)),
      },
      {
        id: "state",
        ...
        currentValue: "choose PR",
        submenu: () =>
          chooseAnd("Change PR state", async (number, back) =>
            choicePanel(
              menu,
              "PR state",
      ...
      },
      {
        ...
        currentValue: "choose PR",
        submenu: () =>
          chooseAnd("Merge PR", async (number, back) =>
            choicePanel(
              menu,
              "Merge method",
              ...
              (method) =>
                menu.actions.mergePullRequest(number, method as "merge" | "squash" | "rebase"),
              back,
            ),
          ),
      },
...
function makeLocal(menu: MenuContext): Component {
  const items: SettingItem[] = [
    {
      id: "dirty-policy",
      label: "Dirty tree policy",
      description: "Refuse is safest; assisted commit offers an explicit commit flow.",
      currentValue: menu.settings.dirtyPolicy,
      values: ["refuse", "offer-commit"],
    },
    {
      id: "unpushed",
      label: "Warn about unpushed commits",
      description: "Warn when the current branch is ahead of its upstream.",
      currentValue: menu.settings.warnUnpushed ? "yes" : "no",
      values: ["yes", "no"],
    },
    {
      id: "confirm",
      label: "Confirm branch changes",
      description: "Ask before checkout, branch, push, commit, or merge operations.",
      currentValue: menu.settings.confirmBranchChanges ? "yes" : "no",
      values: ["yes", "no"],
    },
    {
      id: "refresh-data",
      label: "Auto-refresh GitHub data",
      description: "Refresh issue autocomplete after each session starts.",
      currentValue: menu.settings.autoRefresh ? "yes" : "no",
      values: ["yes", "no"],
    },
  ]
  return new SettingsPanel(
    "Local workflow",
    "Configure checkout and safety preferences.",
    items,
    (id, value) => {
...
function makeStatus(menu: MenuContext): Component {
  return new SettingsPanel(
    "GitHub status",
    "Show current repository and authentication state.",
    [
      {
        id: "status",
        label: "Repository status",
        description: "Show Git, GitHub, authentication, PR, and check status.",
        currentValue: "view",
        submenu: () =>
          readPanel(menu, "GitHub status", "Current repository and authentication state.", () =>
            menu.actions.showStatus(),
          ),
      },
    ],
    () => undefined,
...
export function createGithubMenu(
  ...
  return new SettingsPanel(
    "GitHub",
    "Choose a section. Enter opens it; Esc goes back.",
    [
      {
        id: "workflow",
        label: "PR workflow",
        description: "Checkout and restore pull-request code.",
        currentValue: "open",
        submenu: (_value, back) => makeWorkflow({ ...menu, done: back }),
      },
      {
        id: "browse",
        label: "Browse GitHub",
        description: "Inspect PRs, diffs, issues, and repository links.",
        currentValue: "open",
        submenu: (_value, back) => makeBrowse({ ...menu, done: back }),
      },
      {
        id: "checks",
        label: "Checks and CI",
        description: "Review or wait for GitHub checks.",
        currentValue: "open",
        submenu: (_value, back) => makeChecks({ ...menu, done: back }),
      },
      {
        id: "repository",
        label: "Repository actions",
        description: "Create PRs and perform confirmation-gated actions.",
        currentValue: "open",
        submenu: (_value, back) => makeRepository({ ...menu, done: back }),
      },
      {
        id: "local",
        label: "Local workflow",
        description: "Configure checkout and safety preferences.",
        currentValue: "configure",
        submenu: (_value, back) => makeLocal({ ...menu, done: back }),
      },
      {
        id: "status",
        label: "GitHub status",
        description: "Show current repository and authentication state.",
        currentValue: "view",
        submenu: (_value, back) => makeStatus({ ...menu, done: back }),
      },
    ],
    () => undefined,
    done,
