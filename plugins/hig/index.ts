...
import { sliceByColumn, truncateToWidth, visibleWidth, type EditorOptions, type EditorTheme, type TUI } from "@earendil-works/pi-tui";
...
class ChatViewport {
  ...
  private copyFlash: { bounds: { start: MousePoint; end: MousePoint }; startedAt: number } | undefined;

  constructor(private readonly container: RenderableContainer, private readonly theme: Theme) {
  ...
  private getSelectionBounds(): { start: MousePoint; end: MousePoint } | undefined {
    ...
    const startBeforeEnd = this.selectionStart.y < this.selectionEnd.y ||
      (this.selectionStart.y === this.selectionEnd.y && this.selectionStart.x <= this.selectionEnd.x);
  ...
  private highlightLine(line: string, row: number): string {
    ...
    const from = row === activeBounds.start.y ? Math.max(range.start, activeBounds.start.x) : range.start;
    ...
    const after = sliceByColumn(line, from + selectedWidth, Math.max(0, visibleWidth(line) - from - selectedWidth));
    ...
    const flashBackground = flashAge < 140
      ? "selectedBg"
      : flashAge < 280
        ? "customMessageBg"
        : "userMessageBg";
