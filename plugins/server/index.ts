...
function statusText(status: Awaited<ReturnType<ServerManager["status"]>>): string {
  if (!status.running)
    return `Pi host: stopped${status.enabled ? " (automatic startup enabled)" : ""}`
...
async function showPairingQr(
  ...
  if (!ctx.hasUI || ctx.mode !== "tui") {
    ctx.ui.notify(
      `Pi host started at ${endpoint}, but the pairing QR requires TUI mode.`,
      "warning",
    )
    return
  }
  ...
  await ctx.ui.custom<void>(
    (tui, theme, _keybindings, done) => {
      ...
      container.addChild(
        new Text(theme.fg("muted", "Scan this QR code with the mobile app."), 1, 0),
      )
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (command === "disable" || command === "stop") {
          ...
          ctx.ui.notify(
            stopped
              ? "Pi host stopped and automatic startup disabled."
              : "Pi host was not running.",
            "info",
          )
          return
        }
        ...
        if (command === "logout") {
          ...
          ctx.ui.notify(
            deleted
              ? "Pi host stopped and its saved auth token was deleted."
              : "No saved Pi host auth token was found.",
            "info",
          )
          return
        }
        ...
        if (command !== "status") {
          ctx.ui.notify(
            "Usage: /server start | enable | stop | disable | status | logout",
            "warning",
          )
          return
        }
        ...
        ctx.ui.notify(
          `Pi host operation failed: ${error instanceof Error ? error.message : String(error)}`,
          "error",
        )
      }
    },
