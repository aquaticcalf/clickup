...
async function waitForCdp(port: number, child: ChildProcess, signal?: AbortSignal): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (child.exitCode !== null)
      throw new Error(`Brave exited before CDP became available (code ${child.exitCode}).`)
...
async function launchBrave(signal?: AbortSignal): Promise<ManagedBrowser> {
  ...
  try {
    ...
    const browser = await withAbort(chromium.connectOverCDP(`http://127.0.0.1:${port}`), signal)
