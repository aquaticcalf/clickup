...
function parseHtmlResults(html: string, selectors: {
  cards: string;
  title: string;
  snippet: string;
}): RawHit[] {
...
const mojeek: Backend = {
  ...
  async search(query, _recency, signal) {
    const html = await fetchText(`https://www.mojeek.com/search?q=${encodeURIComponent(query)}`, {}, signal);
    return parseHtmlResults(html, { cards: "ul.results > li", title: "h2 a", snippet: "p.s" }).slice(0, 10);
  },
};
...
const yahoo: Backend = {
  name: "yahoo",
  async search(query, _recency, signal) {
    const html = await fetchText(`https://search.yahoo.com/search?p=${encodeURIComponent(query)}`, {}, signal);
    return parseHtmlResults(html, {
...
const bing: Backend = {
  ...
  async search(query, recency, signal) {
    ...
    return parseHtmlResults(html, { cards: ".b_algo", title: "h2 a", snippet: ".b_caption, p" }).slice(0, 10);
  },
};
...
const wikipedia: Backend = {
  name: "wikipedia",
  async search(query, _recency, signal) {
    const params = new URLSearchParams({ action: "opensearch", profile: "fuzzy", limit: "5", search: query, format: "json", origin: "*" });
    ...
    return (data[1] ?? []).map((title, index) => ({
      title,
      url: data[3]?.[index] ?? `https://en.wikipedia.org/wiki/${encodeURIComponent(title.replace(/ /g, "_"))}`,
...
function titleRelevance(query: string, title: string): number {
  ...
  return terms.length === 0 ? 0 : terms.filter((term) => haystack.includes(term)).length / terms.length;
}
